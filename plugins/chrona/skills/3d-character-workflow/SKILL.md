...
This loaded Chrona plugin bundle is version `0.1.1`. On the first use of this skill in each new task, make one non-blocking update check for the current host only:
...
- Only when the refreshed manifest contains a valid semantic version greater than `0.1.1`, append one short, non-blocking notice that names the available version and asks the user to say `更新插件`. Continue the character request without waiting.
